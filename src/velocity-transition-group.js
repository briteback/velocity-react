...
import { each, extend, forEach, isEqual, keys, omit, map } from 'lodash';
...
class VelocityTransitionGroup extends React.Component {
  ...
  render() {
    ...
    var transitionGroupProps = omit(this.props, keys(VelocityTransitionGroup.propTypes));
  ...
  _runAnimation(entering, queue, animationProp) {
    ...
    if (entering) {
      if (!isEqual(this.props.enterShowStyle, {display: ''})
